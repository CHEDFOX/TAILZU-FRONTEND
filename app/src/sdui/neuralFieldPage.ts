 ...
 *
 * `cfg.tuning` is every constant the drawing used to carry in code — how many
 * branches a system grows, how fast a pulse travels, how hard the bloom glows
 * — as one object merged over the table T below. Absent, T is exactly what
 * this page always drew. The app fills it from the node's `tuning` prop over
 * the global ui.NeuralField.tuning knob.
 */
...
/* THE TUNING TABLE. Defaults are what the field always was; CFG.tuning is
   merged over it key by key, so a server can move one number and leave the
   rest. Ranges are [lo,hi] pairs handed to rnd(). */
var T={
  steps:30,
  hubWeight:[1,0.8,0.5,0.3], hubTufts:[22,8,4,2], hubDots:[120,36,14,6],
  tuftDist:[0.1,0.8], tuftLen:[0.25,0.6], tuftWidth:[0.4,0.9], dotSize:[0.4,1.3],
  highwayDetour:1.45, highwaySpread:0.034, tractSpread:0.95, tractBend:[0.08,0.22],
  coreR:0.045, majorsMin:6, majorsExtra:4, majorDist:[0.11,0.16], majorR:[0.017,0.024],
  ringLinkP:0.6, minorsMin:2, minorsExtra:3, minorDist:[0.06,0.11], minorR:[0.009,0.014],
  microMin:1, microExtra:3, microDist:[0.03,0.06], microR:[0.004,0.007],
  crossLinks:4, crossDist:0.2,
  systemSpacing:0.2, placeTries:60, systemDist:[0.16,0.34], regionCount:[0.55,1],
  regionLinkP:[0.35,1], regionLinkExtraP:0.5, majorLinkDist:0.12, majorLinkP:0.4, majorLinkGrowth:[0.3,1],
  fibresHighway:[34,60], fibresDirect:[3,24], fibresGrowth:[0.34,1],
  slabBlur:4.2, parallax:[1,5], grainSize:160, grainAlpha:0.02,
  fibreAlpha:[0.13,0.36], fibreWidth:[0.3,0.7], alphaEase:5,
  refractory:[0.55,0.95], offBiasP:0.22, shareBias:[0.45,0.75], shareOther:[0.15,0.3],
  pulseGain:1.15, pulseSpeed:[0.85,1.3], pulseDelay:[0,0.26], highwaySlow:0.7,
  chargeDecay:0.6, heatDecay:0.08, heatGain:0.05,
  newTractP:0.1, growRate:[0.32,0.55], sproutsOnStart:5, growIdle:0.15,
  sproutTraining:2.2, maxGrowingTraining:12, sproutIdle:0.1, maxGrowingIdle:2,
  sway:0.07, idleKick:2.4, listenKick:5, listenKickLevel:14, listenSec:2.8,
  thinkKick:3.5, thinkSec:1.4, thinkSecRand:0.8, speakCoreKick:2.2, speakKick:1.6, speakSec:2.4,
  glowR:2.2, glowPad:4, glowHead:0.38, glowSignal:0.28,
  sparkR:9, sparkAlpha:0.5, headR:1.3, trail:6, trailStep:0.02,
  bloomBlur:5, bloomScale:4, idleFps:32, maxDt:0.05, resizeTolerance:0.25, maxDpr:2
};
try{var TU=CFG.tuning;if(TU&&typeof TU==="object")for(var tk in TU)if(TU[tk]!=null)T[tk]=TU[tk]}catch(e){}
var rnd=function(a,b){return a+Math.random()*(b-a)},
    rr=function(r){return rnd(r[0],r[1])},
...
var FOCAL=CFG.focal!=null?CFG.focal:0.35, STEPS=T.steps;
...
function makeHub(x,y,z,r,level,hue){
  var h={x:x,y:y,z:z,r:r,level:level,hue:hue,core:level===0,tufts:[],dots:[],links:[],
         charge:0,refr:0,heat:0,weight:T.hubWeight[level]};
  var n=T.hubTufts[level],i,k;
  for(i=0;i<n;i++){
    var a=rnd(0,TAU),d=r*rr(T.tuftDist),pts=[x+Math.cos(a)*d,y+Math.sin(a)*d],aa=rnd(0,TAU),len=r*rr(T.tuftLen);
    for(k=0;k<4;k++){aa+=rnd(-0.5,0.5);pts.push(pts[pts.length-2]+Math.cos(aa)*len/4,pts[pts.length-1]+Math.sin(aa)*len/4)}
    h.tufts.push({pts:pts,w:rr(T.tuftWidth)});
  }
  var nd=T.hubDots[level];
  for(i=0;i<nd;i++){var a2=rnd(0,TAU),d2=r*Math.sqrt(Math.random())*1.05;
    h.dots.push(x+Math.cos(a2)*d2,y+Math.sin(a2)*d2,rr(T.dotSize))}
...
function makeTract(a,b,weight,route){
  ...
  if(route&&highways.length){
    ...
    for(i=0;i<highways.length;i++){var hw=highways[i],
      ...
      if(cost<direct*T.highwayDetour&&(!best||cost<best.cost))best={hw:hw,ia:ra[0],ib:rb[0],cost:cost}}
    if(best){var lo=Math.min(best.ia,best.ib),hi=Math.max(best.ia,best.ib),
      ...
      via=true}}
  if(!pl){var ab=arcBez([a.x,a.y],[b.x,b.y],(Math.random()<0.5?1:-1)*rr(T.tractBend));
  ...
  var t={a:a,b:b,weight:weight,z:(a.z+b.z)/2,viaHighway:via,
         ...
         spread:via?W*T.highwaySpread:Math.min(a.r,b.r)*T.tractSpread,fibres:[]};
...
function system(cx,cy,sc,z,hue,ax,el){
  var core=makeHub(cx,cy,z,W*T.coreR*sc,0,hue),majors=[],minors=[],
      NM=T.majorsMin+((Math.random()*T.majorsExtra)|0),a0=rnd(0,TAU),i;
  for(i=0;i<NM;i++){var a=a0+(i/NM)*TAU+rnd(-0.16,0.16),rd=W*rr(T.majorDist)*sc,
      ux=Math.cos(a-ax)*rd*el,uy=Math.sin(a-ax)*rd,
      px=cx+ux*Math.cos(ax)-uy*Math.sin(ax),py=cy+ux*Math.sin(ax)+uy*Math.cos(ax),
      m=makeHub(px,py,clamp(z+rnd(-0.25,0.25),-1,1),W*rr(T.majorR)*sc,1,hue+rnd(-30,30));
    m.ang=Math.atan2(py-cy,px-cx);majors.push(m);makeTract(core,m,1)}
  for(i=0;i<NM;i++)if(Math.random()<T.ringLinkP)makeTract(majors[i],majors[(i+1)%NM],0.26);
  majors.forEach(function(m){
    var n=T.minorsMin+((Math.random()*T.minorsExtra)|0),k;
    for(k=0;k<n;k++){var a=m.ang+rnd(-0.95,0.95),d=W*rr(T.minorDist)*sc,
        mi=makeHub(m.x+Math.cos(a)*d,m.y+Math.sin(a)*d*1.15,clamp(m.z+rnd(-0.25,0.25),-1,1),
                   W*rr(T.minorR)*sc,2,m.hue+rnd(-12,12));
      mi.ang=a;minors.push(mi);makeTract(m,mi,0.55);
      var nn=T.microMin+((Math.random()*T.microExtra)|0),q;
      for(q=0;q<nn;q++){var b=a+rnd(-1.2,1.2),dd=W*rr(T.microDist)*sc,
        mc=makeHub(mi.x+Math.cos(b)*dd,mi.y+Math.sin(b)*dd*1.1,clamp(mi.z+rnd(-0.25,0.25),-1,1),
                   W*rr(T.microR)*sc,3,mi.hue+rnd(-10,10));
        makeTract(mi,mc,0.3)}}});
  for(i=0;i<T.crossLinks;i++){var a3=pick(minors),b3=pick(minors);
    if(a3!==b3&&Math.hypot(a3.x-b3.x,a3.y-b3.y)<W*T.crossDist*sc)makeTract(a3,b3,0.2)}
...
function build(){
  ...
  var regions=(CFG.regions||[]).map(function(R){return {
      ...
      n:Math.max(2,Math.round(R.n*lerp(T.regionCount[0],T.regionCount[1],G))),z:R.z,sc:R.sc}}),
      placed=[],i,j,k;
  regions.forEach(function(R){
    var mine=[];
    for(i=0;i<R.n;i++){var x,y,tries=0;
      do{var a=rnd(0,TAU),d=i===0?0:W*rr(T.systemDist);
        x=R.seat[0]+Math.cos(a)*d;y=R.seat[1]+Math.sin(a)*d*1.25}
      while(placed.some(function(p){return Math.hypot(p[0]-x,p[1]-y)<W*T.systemSpacing})&&++tries<T.placeTries);
      placed.push([x,y]);
  ...
  for(i=0;i<regions.length;i++)for(j=i+1;j<regions.length;j++){
    var n2=(Math.random()<lerp(T.regionLinkP[0],T.regionLinkP[1],G)?1:0)+(Math.random()<T.regionLinkExtraP*G?1:0);
  ...
  for(i=0;i<cores.length;i++)for(j=i+1;j<cores.length;j++){
    ...
    for(p1=0;p1<A.length;p1++)for(p2=0;p2<B.length;p2++)
      if(Math.hypot(A[p1].x-B[p2].x,A[p1].y-B[p2].y)<W*T.majorLinkDist&&Math.random()<T.majorLinkP*lerp(T.majorLinkGrowth[0],T.majorLinkGrowth[1],G))
        makeTract(A[p1],B[p2],0.28)}
  tracts.forEach(function(t){
    /* The thickness of a tract IS how well travelled it is. */
    var n=Math.round((t.viaHighway?rr(T.fibresHighway):lerp(T.fibresDirect[0],T.fibresDirect[1],t.weight))*lerp(T.fibresGrowth[0],T.fibresGrowth[1],G))
          +((Math.random()*3)|0),i2;
...
var SLABS=[],i0;
for(i0=0;i0<5;i0++){var lo=-1+i0*0.4,hi=lo+0.4,zc=(lo+hi)/2,blur=Math.abs(zc-FOCAL)*T.slabBlur;
  SLABS.push({lo:lo,hi:i0===4?1.01:hi,blur:blur,
    scale:clamp(1/(1+blur*0.5),0.4,1),par:lerp(T.parallax[0],T.parallax[1],(zc+1)/2)})}
...
var GS=T.grainSize,grain=document.createElement("canvas");grain.width=grain.height=GS;
(function(){var g=grain.getContext("2d"),im=g.createImageData(GS,GS),i;
...
/* HOW FAR BACK IT SITS, LIVE. alpha used to be baked into the plates, so a
   screen that wanted the field brighter than the one before had to throw the
   whole drawing away and bake it again — a black frame, then a new network,
   at the one moment (slide to begin) that should be continuous. Now the plates
   are baked at full strength and alpha is applied as the frame is drawn, and
   it EASES to wherever it was last told to go: the field brightens into a
   session over a quarter of a second instead of being replaced. */
var A_TO=CFG.alpha==null?1:clamp(Number(CFG.alpha),0,1),A=A_TO;
function fibreLook(f){var nz=(f.z+1)/2;return [lerp(T.fibreAlpha[0],T.fibreAlpha[1],nz),lerp(T.fibreWidth[0],T.fibreWidth[1],nz)]}
...
function bake(){
  SLABS.forEach(function(S,i){
    ...
    hubs.forEach(function(h){if(h.z<S.lo||h.z>=S.hi)return;
      ...
      gl.addColorStop(0,rgba(hsl(h.hue,80,62),h.core?0.10:0.03*h.weight));
      gl.addColorStop(1,rgba(col,0));
      g.fillStyle=gl;g.beginPath();g.arc(h.x,h.y,h.r*1.7,0,TAU);g.fill();
      g.strokeStyle=rgba(col,lerp(0.18,0.4,nz));
      ...
      g.fillStyle=rgba(hsl(h.hue,70,80),lerp(0.3,0.65,nz));
  ...
  bakedW=W;bakedH=H;bakedDPR=DPR;
  dirty=false}
function bakeOne(f){
...
function fire(h){
  h.heat=1;h.charge=0;h.refr=rr(T.refractory);
  h.links.forEach(function(t){
    ...
    if(!withBias&&Math.random()>T.offBiasP)return;
    if(!t.fibres.length)return;
    var share=withBias?rr(T.shareBias):rr(T.shareOther),
        n=Math.max(1,Math.round(t.fibres.length*share)),
        fromA=t.a===h,gain=(T.pulseGain*(0.7+0.5*t.weight))/n,i;
    for(i=0;i<n&&pulses.length<MAXP;i++){
      pulses.push({f:t.fibres[(Math.random()*t.fibres.length)|0],s:fromA?0:1,d:fromA?1:-1,
        sp:rr(T.pulseSpeed)*(t.viaHighway?T.highwaySlow:1),delay:rr(T.pulseDelay),to:other,gain:gain,a:rnd(0.7,1)})}})}
function tick(dt){
  hubs.forEach(function(h){
    if(h.refr>0)h.refr-=dt;
    if(h.charge>0.001)h.charge*=Math.pow(T.chargeDecay,dt);
    if(h.heat>0.001)h.heat*=Math.pow(T.heatDecay,dt);
    if(h.refr<=0&&h.charge>=1)fire(h)});
  var i;for(i=pulses.length-1;i>=0;i--){var p=pulses[i];
    if(p.delay>0){p.delay-=dt;continue}
    p.s+=p.d*p.sp*dt;
    if(p.s<0||p.s>1){p.to.charge+=p.gain;p.to.heat=Math.min(1,p.to.heat+T.heatGain);pulses.splice(i,1)}}}
function kick(h){if(h&&h.refr<=0)fire(h)}

function sproutFibre(){
  var tr=null;
  if(Math.random()<T.newTractP){var a=pick(hubs),b=pick(hubs);
  ...
  var f=makeFibre(tr);f.len=0.02;f.rate=rr(T.growRate);growing.push(f)}
...
window.tz=function(msg){
  try{var d=typeof msg==="string"?JSON.parse(msg):msg;
    ...
    if(d.training!=null){var t=!!d.training;
      if(t&&!training){var i;for(i=0;i<T.sproutsOnStart;i++)sproutFibre()}
      training=t;if(!t)setMode("idle")}
    if(d.run!=null)setRun(!!d.run);
    /* The two per-screen numbers, retuned in place. A screen that wants the
       field brighter or dimmer says so and it eases there; one that knows the
       network has grown rebuilds it under the last frame, which stays on the
       canvas until the new one is ready — never a black frame between. */
    if(d.alpha!=null)A_TO=clamp(Number(d.alpha),0,1);
    /* The rest of the look that is read per frame, retuned in place too:
       nothing is rebuilt, the next frame simply draws with the new value. */
    if(d.bloom!=null)CFG.bloom=Number(d.bloom);
    if(d.maxPulses!=null)MAXP=Number(d.maxPulses)||MAXP;
    if(d.signal&&d.signal.length===3)SIG=d.signal;
    if(d.head&&d.head.length===3)HEAD=d.head;
    if(d.growth!=null){var g2=clamp(Number(d.growth),0,1),g0=CFG.growth==null?1:clamp(Number(CFG.growth),0,1);
      if(Math.abs(g2-g0)>0.004&&W&&H){CFG.growth=g2;build();bake()}}
  }catch(e){}};
...
function update(dt){
  t0+=dt;modeT+=dt;sway=t0*T.sway;
  A+=(A_TO-A)*Math.min(1,dt*T.alphaEase);if(Math.abs(A_TO-A)<0.002)A=A_TO;
  var i;
  for(i=growing.length-1;i>=0;i--){var f=growing[i];
    f.len=Math.min(1,f.len+f.rate*(training?1:T.growIdle)*dt);
    if(f.len>=1){growing.splice(i,1);fibres.push(f);bakeOne(f)}}
  if(training&&Math.random()<dt*T.sproutTraining&&growing.length<T.maxGrowingTraining)sproutFibre();
  if(!training&&Math.random()<dt*T.sproutIdle&&growing.length<T.maxGrowingIdle)sproutFibre();

  if(!training){if(Math.random()<dt*T.idleKick)kick(pick(peri))}
  else if(mode==="listening"){
    if(Math.random()<dt*(T.listenKick+level*T.listenKickLevel))kick(pick(peri));
    if(modeT>T.listenSec+Math.random())setMode("thinking")}
  else if(mode==="thinking"){
    if(Math.random()<dt*T.thinkKick)kick(pick(cores));
    if(modeT>T.thinkSec+Math.random()*T.thinkSecRand)setMode("speaking")}
  else if(mode==="speaking"){
    if(Math.random()<dt*T.speakCoreKick)kick(cores[0]);
    if(Math.random()<dt*T.speakKick)kick(pick(cores));
    if(modeT>T.speakSec+Math.random())setMode("listening")}
  tick(dt);
  if(dirty)bake()}
...
function render(){
  ...
  SLABS.forEach(function(S,i){
    ...
    ctx.save();ctx.globalAlpha=A;ctx.translate(W/2+dx,H/2+dy);
    ctx.drawImage(plates[i],-W/2,-H/2,W,H);ctx.restore()});
  hubs.forEach(function(h){
    var heat=Math.min(1,h.heat+h.charge*0.35);if(heat<0.04)return;
    var R=h.r*T.glowR+T.glowPad,ga=ctx.createRadialGradient(h.x,h.y,0,h.x,h.y,R);
    ga.addColorStop(0,rgba(HEAD,T.glowHead*heat));
    ga.addColorStop(0.35,rgba(SIG,T.glowSignal*heat));
  ...
  growing.forEach(function(f){
    var l=fibreLook(f);strokeFibre(ctx,f,l[0]*1.6*A,l[1]*1.3);
    var p=fibrePoint(f,f.len),g=ctx.createRadialGradient(p[0],p[1],0,p[0],p[1],T.sparkR);
    g.addColorStop(0,rgba(SIG,T.sparkAlpha));g.addColorStop(1,rgba(SIG,0));
    ctx.fillStyle=g;ctx.beginPath();ctx.arc(p[0],p[1],T.sparkR,0,TAU);ctx.fill();
    ctx.fillStyle=rgba(HEAD,0.95);ctx.beginPath();ctx.arc(p[0],p[1],T.headR,0,TAU);ctx.fill()});
  pulses.forEach(function(p){
    if(p.delay>0)return;
    var nz=(p.f.z+1)/2,k;ctx.beginPath();
    for(k=0;k<=T.trail;k++){var q=fibrePoint(p.f,p.s-p.d*k*T.trailStep);
  ...
  if(CAN_FILTER){
    var bw=Math.max(1,(cv.width/T.bloomScale)|0),bh=Math.max(1,(cv.height/T.bloomScale)|0);
    ...
    ctx.filter="blur("+T.bloomBlur+"px)";ctx.drawImage(bloom,0,0,W,H);ctx.restore()}
  if(grainPat){ctx.save();ctx.globalCompositeOperation="lighter";ctx.globalAlpha=T.grainAlpha;
    ctx.translate((t0*37)%GS|0,(t0*53)%GS|0);
    ctx.fillStyle=grainPat;ctx.fillRect(-GS,-GS,W+GS*2,H+GS*2);ctx.restore()}}
...
var running=true,raf=0,MIN_DT_IDLE=1/T.idleFps;
function setRun(on){
...
function step(now){
  ...
  last=now;
  update(Math.min(T.maxDt,dt));render();
  if(!painted){painted=true;try{window.ReactNativeWebView&&window.ReactNativeWebView.postMessage("painted")}catch(e){}}}

/* A SMALL RESIZE IS NOT A NEW NETWORK.
   Any change of size used to rebuild the whole thing — forty thousand new
   curves, baked again — which is half a second with nothing else moving, and
   a different network on the other side of it. A scrollbar appearing costs
   fifteen pixels, and that was enough. So a change of under a quarter, at
   the same pixel density, scales what is already built to the new size and
   lets the plates stretch with it: nothing is rebuilt, nothing is baked, and
   the same network is simply a little wider. Measured against the size the
   plates were BAKED at, so small changes cannot add up to a blurry drawing —
   a quarter's drift from the last bake is a real resize, and rebuilds. */
var bakedW=0,bakedH=0,bakedDPR=0;
function scalePts(a,sx,sy){var i;for(i=0;i<a.length;i+=2){a[i]*=sx;a[i+1]*=sy}}
function scaleGeometry(sx,sy){
  var sr=(sx+sy)/2;
  hubs.forEach(function(h){h.x*=sx;h.y*=sy;h.r*=sr;
    h.tufts.forEach(function(t){scalePts(t.pts,sx,sy)});
    var k;for(k=0;k<h.dots.length;k+=3){h.dots[k]*=sx;h.dots[k+1]*=sy;h.dots[k+2]*=sr}});
  tracts.forEach(function(t){scalePts(t.path,sx,sy);t.spread*=sr});
  var all=fibres.concat(growing);
  all.forEach(function(f){scalePts(f.pts,sx,sy);
    f.twigs.forEach(function(t){scalePts(t,sx,sy)})});
  highways.forEach(function(hw){scalePts(hw,sx,sy);hw.step*=sr})}
function layout(){
  var w=cv.clientWidth,h=cv.clientHeight;
  if(!w||!h)return;
  var dpr=Math.min(window.devicePixelRatio||1,T.maxDpr),
      sx=W?w/W:1,sy=H?h/H:1,
      small=bakedW&&dpr===bakedDPR&&Math.abs(w/bakedW-1)<T.resizeTolerance&&Math.abs(h/bakedH-1)<T.resizeTolerance;
  W=w;H=h;DPR=dpr;
  cv.width=Math.round(W*DPR);cv.height=Math.round(H*DPR);
  ctx.setTransform(DPR,0,0,DPR,0,0);
  if(!grainPat)grainPat=ctx.createPattern(grain,"repeat");
  if(!small){build();bake()}
  else if(Math.abs(sx-1)>1e-4||Math.abs(sy-1)>1e-4)scaleGeometry(sx,sy)}
