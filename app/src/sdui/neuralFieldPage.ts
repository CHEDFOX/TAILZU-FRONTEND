...
function fibreLook(f){var nz=(f.z+1)/2;return [lerp(0.13,0.36,nz),lerp(0.3,0.7,nz)]}
...
function update(dt){
  ...
  A+=(A_TO-A)*Math.min(1,dt*5);if(Math.abs(A_TO-A)<0.002)A=A_TO;
  var i;
...
function layout(){
  ...
  var dpr=Math.min(window.devicePixelRatio||1,2),
      ...
      small=bakedW&&dpr===bakedDPR&&Math.abs(w/bakedW-1)<0.25&&Math.abs(h/bakedH-1)<0.25;
  W=w;H=h;DPR=dpr;
