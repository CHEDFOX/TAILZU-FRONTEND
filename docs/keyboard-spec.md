...
```json
{
  ...
  "props": {
    ...
    "motion": {
      ...
      "recording": {
        "kind": "dance", "tempo": 1, "reach": 1, "blend": 1.5, "voice": 0.35, "settle": 0.9,
        "score": [
...
- `motion.idle`: `signal` (on `"mark"`) is the splash's sequence on a loop, a timeline of `steps` over `period` seconds. Each step names a shape and the second it lights (`at`): a square or plain line wears the signal `color` for `hold` seconds (on in 60ms, off in 60ms); a dashed line has a bright cluster of its dashes travel its length for `run` seconds — the dashes stay where they are; the cluster spans `width` of the line (0.3 by default), fully lit at its core and soft at its edges, the way the splash runs it between the blocks. `color` defaults to pale on a tinted mark and amber otherwise. `breathe` swells a shape (or `"mark"`, the whole) to `scale` and fades it to `opacity` and back; `hatch` runs a dashed line's dashes along it. All on `period` seconds; all honour the system's reduce-motion setting. On a 36pt key the signal and the breath are what shows; the hatch is for the sizes where dashes resolve.
- `motion.recording`: what the key does while the microphone is open. `{ "kind": "dance", ... }` — one choreography, on one clock, looping. The squares and the dot are the dancers, in chain order left to right (`k`) and by depth from the top; each line's ends are tied to the nearest dancer and turn with it. The `score` is a series of movements, each with a figure (`move`), a length (`for`, seconds) and a `beat` (seconds per cycle of the figure); `for` should be a whole number of beats so every movement starts and ends at rest. Movements overlap by `blend` seconds, weighted by a raised cosine, so one flows into the next. The figures: `sway` (the whole turns `turn` degrees either way and breathes by `breathe`; squares counter-turn a little), `wave` (a wave of `lift` runs down the chain with `lag` radians between neighbours; squares tilt `tilt` degrees with the slope), `carousel` (the whole turns `turns` times, eased so it starts and stops without a jolt, while each dancer circles on an `epicycle` a quarter turn behind the last), `fold` (the whole draws in by `depth` and out again, squares turning `turn` degrees, alternating), `pendulum` (the whole swings `swing` degrees about the topmost dancer, each deeper dancer `lag` behind), `eight` (each dancer traces a figure of eight of `size`, `lag` behind the last, tilting `tilt`), `spiral` (the whole turns `turns` times while breathing out and in by `breathe`). Sizes are fractions of the artboard's short side. The keyboard chases the pose on critically damped springs and stretches each square along its own speed. The live microphone level scales the tempo and the reach by `voice`, and a rise is an accent, one breath of the whole; `tempo` and `reach` scale the whole score. On stop the pose goes home, within `settle` seconds, and lands exactly; the same view stays mounted throughout and the idle motion resumes. A figure a build does not know is skipped. `"particles"` — the mark bursts into the dot sim and springs back on stop — or `"none"`. A build before the dance reads a name only and shows its particles.
