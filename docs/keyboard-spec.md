...
- `motion.recording`: what the key does while the microphone is open. `{ "kind": "disperse", ... }` — everything but the wave leaves. Every shape except the kept one (`keep`, the id of a dashed line) is a part: at the start of a recording it flies straight out from the artboard's centre, turning `spin` degrees (alternating direction), to `out` key-radii away, past the rim and out of sight, on a critically damped spring so it leaves from rest and gathers speed. The kept line is the wave: it eases to the centre (`centre`) and grows by `wave.lift`, and its dashes become bars, one plain line per dash, whose height follows the live microphone level — a slow ripple down the line plus a little grain, scaled by the level, up to `wave.rise` more than the line's width — while the bright cluster of the signal keeps running along it every `wave.run` seconds (a touch faster the louder it gets), `wave.width` of the line wide, fully lit at its core and soft at its edges. On stop every part flies back on a quicker spring and lands exactly where it began, within `settle` seconds; the bars fade back into the plain link as it returns; the same view stays mounted throughout and the idle motion resumes. `"particles"` — the mark bursts into the dot sim and springs back on stop — or `"none"`. A build before the dispersal reads a name only and shows its particles.
